...
- `worked_example` — A fully worked-out solution to a SIMILAR but DIFFERENT problem that demonstrates the same technique used in this step. The purpose is analogical reasoning: the student sees how the method works on a parallel problem and transfers that understanding.
  Rules:
  - Change the numbers, variables, or physical context so the answer cannot be copied directly.
  - Show every line of work, not just the final result.
  - Keep the same mathematical structure/technique as the real step.
