...
- **Canvas architecture**: New canvas lives in `Features/Canvas/` with centralized `CanvasViewModel` (`@Observable`). UIKit-based `CanvasContainerView` handles PDF+PencilKit (zoom, drawing, dark mode color inversion). Toolbar popovers use `PopoverCard` from `Core/Components/` with arrow offset positioning. Canvas is presented as fullscreen overlay in `DashboardView` via `canvasDocument` state. Old canvas (`Views/Canvas/`) is NOT compiled.
